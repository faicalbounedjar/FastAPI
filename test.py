...
def get_embedding():
    ...
    query_embeddings = get_weightedmean_embedding(tokenize_with_specb(query, is_query=True), model)
    ...
    return {"Cosine similarity between "+query[0]+" and "+docs[0]+"":(cosine_sim_0_1)}
...
def get_results():
    for q in query:
        for doc in docs:
            ...
            continuation_enc = tokenizer.encode(q, add_special_tokens=False)
            ...
            return {"log":logprobs,"continuation_enc":continuation_enc,"tensor":torch.tensor(continuation_enc).unsqueeze(-1)}
            #logprobs = torch.gather(logprobs, 1, torch.tensor(continuation_enc).unsqueeze(-1)).squeeze(-1)
            #score = torch.sum(logprobs)
            
            #result.append({"query" : q , "documents" : doc ,"Score": score})
